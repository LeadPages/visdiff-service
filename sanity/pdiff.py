...
def gen_p_diff(img1, img2):
    ...
    :param img1: string name of file1 (test image)
    :param img2: string name of file2 (baseline image)
    :return: tuple(diff_count, diff_percent) where diff_count is the
             raw number of pixels that did not match the baseline and
             diff_percent is the percentage of diff_count/total_pixels
    """
...
def is_masked(pixel):
    """
    determine if the supplied pixel is covered by a mask (purple cover)
    :param pixel: tuple(R, G, B) where R/G/B are in range 0 - 255
    :return: boolean indicating that the pixel is masked (True) or not (False)
    """
...
def pixels_are_different(pixel1, pixel2):
    ...
    configurable
    :param pixel1: tuple(R, G, B) where R/G/B are in range 0 - 255
    :param pixel2: tuple(R, G, B) where R/G/B are in range 0 - 255
    :return: boolean indicating that the test pixel (pixel1) differs from the
             baseline pixel (pixel2) by more than 40 (True) or less than 40 (False)
    """
    DIFF_THRESHOLD = 40
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print "Please specify both ImageA (test) and ImageB (baseline)" \
